...
(function (PKT_EXT) {
    ...
    var reminderContainer = void 0,
        mouseInside = void 0,
        mainContainer = void 0,
        initalPosition = void 0,
        interfaceBuilt = void 0,
        final_reminder = void 0,
        video_reminder = void 0;
    ...
    var startScrollCloseTimer = function startScrollCloseTimer() {
        var distanceScrolled = Math.abs($(document).scrollTop() - initalPosition);
        if (distanceScrolled < 3200 || mouseInside) return;
        window.removeEventListener('scroll', startScrollCloseTimer);
        startCloseTimer(500);
    };
    ...
    var closeReminder = function closeReminder() {
        stopCloseTimer();
        var main = $(mainContainer);
        if (main && main.length) main.addClass('pkt_ext_reminder_inactive');
    };
    ...
    var checkScrollOpen = function checkScrollOpen() {
        var distanceScrolled = Math.abs($(document).scrollTop() - initalPosition);
        if (distanceScrolled < 300) return;

        sendMessage({ action: 'checkReminders' }, function (response) {
            if (response.show_reminder) showReminder();
        });
        window.removeEventListener('scroll', checkScrollOpen);
    };
    /*====================================
    =            Initializers            =
    ====================================*/
    ...
    var buildReminder = function buildReminder(final) {
        final_reminder = final;
        mainContainer = document.createElement('div');
        mainContainer.id = 'pkt_ext_master';
        mainContainer.className = 'pkt_ext_container pkt_ext_reminder_inactive pkt_ext_container_inactive';

        document.body.appendChild(mainContainer);

        reminderContainer = document.createElement('div');
        reminderContainer.id = 'pkt_ext_reminder_plate';

        mainContainer.appendChild(reminderContainer);

        interfaceBuilt = true;
        initalPosition = $(document).scrollTop();
        window.addEventListener('scroll', checkScrollOpen);
    };

    var setSaveAction = function setSaveAction() {
        $('.pkt_ext_reminder_button').on('click', function () {
            closeReminder();
            sendMessage({ action: 'saveFromReminder', video: video_reminder, final: final_reminder });
        });
    };

    var setCloseAction = function setCloseAction() {
        $('#pkt_ext_reminder_dismiss').on('click', function (e) {
            e.preventDefault();
            closeReminder();
            sendMessage({ action: 'closeReminder', video: video_reminder, final: final_reminder });
        });
    };

    var init = function init() {
        sendMessage({ action: 'checkReminders' }, function (response) {
            if (response.show_reminder) buildReminder(response.final_reminder);
        });
    };
    ...
    var showReminder = function showReminder() {
        ...
        sendMessage({ action: 'reminderShown', final: final_reminder, video: video_reminder });
        var copyObject = {
            video: {
                initial_action: 'watch',
                initial_content: 'videos',
                final_action: 'viewing',
                final_content: ['watch', 'video']
            },
            article: {
                initial_action: 'read',
                initial_content: 'articles',
                final_action: 'reading',
                final_content: ['read', 'article']
            }
        };
        var copy = video_reminder ? copyObject['video'] : copyObject['article'];

        var messaging = {
            initial: {
                title: 'No time to ' + copy.initial_action + ' this? Save for later!',
                message: 'Don\'t lose track of ' + copy.initial_content + ' like this one. Have Pocket remember them for you.'
            },
            final: {
                title: 'Want a cleaner ' + copy.final_action + ' experience?',
                message: 'Save this to Pocket and ' + copy.final_content[0] + ' this ' + copy.final_content[1] + ' without distractions.',
                final_reminder: final_reminder
            }
        };

        var data = final_reminder ? messaging.final : messaging.initial;

        reminderContainer.innerHTML = PKT_EXT.TEMPLATES.reminder(data);
        setSaveAction();
        setCloseAction();

        setTimeout(function () {
            // So Greasy. Timeouts can suck it.  Required to allow css animations to trigger.
            mainContainer.classList.remove('pkt_ext_reminder_inactive');
            initAutoCloseEvents(mainContainer);
        }, 100);
    };

    var saveReminder = function saveReminder(isVideo) {
        video_reminder = isVideo;
        if (!interfaceBuilt) init();
    };

    PKT_EXT.REMINDER = { saveReminder: saveReminder, closeReminder: closeReminder };
})(PKT_EXT || {}); // eslint-disable-line semi
