...
(function(PKT_EXT){
    ...
    let should_recent, tab_count, active_new, show_notification = true
    let newTabNotificationElement
    const sendMessage = PKT_EXT.INTERFACE.sendMessage

    const newTabExtensionURL = 'https://chrome.google.com/webstore/detail/pocket-new-tab/mlnnopicjonfamklpcdfnbcomdlopmof?authuser=1'
    const newTabLearnMoreURL = 'https://help.getpocket.com/article/1141-using-the-pocket-new-tab-extension-in-chrome'
    ...
    const injectContainer = function(force){
        $(function(){
            ...
            if(googleTiles){
                ...
                $('body').append(PKT_EXT.TEMPLATES.newtab({ newTabExtensionURL, newTabLearnMoreURL }))
    ...
    const setActions = function (){
        ...
        $('.pkt_ext_newtab_button_confirm').on('click', function(e){
            e.preventDefault()
            ...
            window.location.href = newTabExtensionURL
        })

        $('.pkt_ext_newtab_button_deny').on('click', function(e){
            e.preventDefault()
            ...
            sendMessage({
                ...
                newTabNotificationElement.addClass('hidden')
            })
        })
    ...
    const buildNewTab = function( settings ){
        ...
        .spread(function (recs, trends) {
            ...
            newTabNotificationElement = $('.pkt_ext_newtab_notification');

            if(!settings.should_show_notification){
                newTabNotificationElement.addClass('hidden')
            }
    ...
    const returnShowNewTabNotification = function(){
        let showNotification = Q.defer()
        sendMessage({
            action: 'getSetting',
            key: 'show_notification'
        }, response => {
            ...
            showNotification.resolve(show_notification)
        })
        return showNotification.promise
    }
    ...
    const getNewtabSettings = function( setup ){
        ...
        Q.allSettled([
            ...
            returnShowNewTabNotification()
        ])
        .spread(function ( count, optIn, tiles, active, notification) {
            ...
            var settings = {
                ...
                should_show_notification: notification.value
            }
