    ...
    function getFeatures(callback){
        ...
        PKT_EXT.API.getFeatures({
            success: function(response) {
                ...
                setReminderTest(response.features)
...
===========================================================================*/
    ...
    function shouldShowReminder(reminders_shown, activationTime){

        const reminders_complete    = getSetting('reminders_complete') || false
        if(reminders_complete)  return false

        if(!reminders_shown) {
            if(getSetting('reminders') === 'chrome_tooltip_existing_user_v3') return true
            if(Date.now() - activationTime > 86400000) return true // 1 day elapsed 86400000.00 millisecond
            else return false
        }

        var elapsedTime = Date.now() - reminders_shown
        if(elapsedTime > 604800000) return true // 7 days elapsed 604800000.00 millisecond

        return false
    }

    function checkReminders(sendResponse, sender, request){
        log.debug('Checking Reminders')
        if(!PKT_EXT.API.isAuthorized() ||
            getCurrentLanguageCode() !== 'en' ||
            getSetting('reminders_complete')) {
            return sendResponse({reminders_active: false})
        }

        const inReminderTest = getSetting('reminders')
        if(!inReminderTest){
            log.debug('Checking Features')
            return getFeatures(function(){checkReminders(sendResponse, sender, request)})
        }

        if(getSetting('reminders') !== 'chrome_tooltip_new_user_v3' &&
            getSetting('reminders') !== 'chrome_tooltip_existing_user_v3') {
            return sendResponse({reminders_active: false})
        }

        let activationTime = getSetting('reminder_activated')
        if(!activationTime) {
            activationTime = Date.now()
            setSetting('reminder_activated', activationTime)
        }

        const reminders_shown   = getSetting('reminder_shown') || false
        const show_reminder     = shouldShowReminder(reminders_shown, activationTime)

        sendResponse({
            final_reminder: (reminders_shown),
            show_reminder: show_reminder
        })
        return true
    }

    function reminderShown(request){
        if(request.final) setSetting('reminders_complete', true)
        setSetting('reminder_shown', Date.now())
    }

    function sendReminderAnalytics(request, sender, action){
        var analyticsObject = {
            view        : 'extension_tooltip',
            section     : (request.final) ? 'save_tooltip_2' : 'save_tooltip_1',
            identifier  : (action) ? action : null,
            cxt_domain  : domainForURL(sender.url),
            cxt_video   : (request.video) ? 1 : 0
        }
        PKT_EXT.API.sendAnalyticsNewtab(analyticsObject)
    }

    function trackToReminderTest(){
        const reminders_tracked    = getSetting('reminders_tracked')
        if(reminders_tracked) return

        var guid        = getSetting('guid')
        var token       = getSetting('oauth_token')
        var test        = getSetting('reminders')

        if(test !== 'control') {
            PKT_EXT.API.sendAbTestTrack(guid, token, test, 'show_tooltip', function(){
                setSetting('reminders_tracked', true)
            })
        } else {
            PKT_EXT.API.sendAbTestTrack(guid, token, 'chrome_tooltip_new_user_v3', 'control' )
            PKT_EXT.API.sendAbTestTrack(guid, token, 'chrome_tooltip_existing_user_v3', 'control', function(){
                setSetting('reminders_tracked', true)
            })
        }
...
===========================================================================*/
    ...
    addMessageListener(function messageListenerCallback(request, sender, sendResponse) {
        ...
        switch(request.action){
            ...
            case 'checkReminders':
                checkReminders(sendResponse, sender, request)
                return true

            case 'reminderShown':
                log.debug('Reminder Shown')
                sendReminderAnalytics(request, sender, false)
                reminderShown(request)
                return true

            case 'saveFromReminder':
                sendReminderAnalytics(request, sender, 'click')
                saveLinkToPocket(sender.tab, request)
                return true

            case 'closeReminder':
                sendReminderAnalytics(request, sender, 'dismiss')
                return true
